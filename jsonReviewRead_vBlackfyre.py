...
class Review:
    ...
    def getFavorableRating(self):
        return float(self.upVotes) / float(self.totalVotes)
...
def readInReviewRaw(fileName, preCalcFileName, minVoteThreshold, minReviewLength):
    ...
    print("Reviews in Pre-Calculated File: " + str(countIn) + " reviews")
    print("Reviews Eliminated: " + str(countOut) + " reviews")

    return reviewsList


# Read in review data from pre-calculated file
def readInReviewPreCalc(fileName, minVoteThreshold, minReviewLength):
    reviewsList = []
    ...
    for line in open(fileName, 'r'):
        ...
        # Initialize data components into review class
        reviewEntry = Review(reviewerID, productID, upVotes, totalVotes, reviewText, unixReviewTime, overallRating)
        reviewsList.append(reviewEntry)
